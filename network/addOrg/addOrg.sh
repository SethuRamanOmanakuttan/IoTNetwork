...
function generateOrg2() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_CA_BASE} -f $COMPOSE_FILE_CA_ORG2 up -d 2>&1
...
function Org2Up () {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    DOCKER_SOCK=${DOCKER_SOCK} ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_BASE} -f $COMPOSE_FILE_ORG2 -f ${COMPOSE_FILE_COUCH_BASE} -f $COMPOSE_FILE_COUCH_ORG2 up -d 2>&1
  else
    DOCKER_SOCK=${DOCKER_SOCK} ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_BASE} -f $COMPOSE_FILE_ORG2 up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
...
COMPOSE_FILE_COUCH_ORG2=compose/${CONTAINER_CLI}/docker-compose-couch-org2.yaml
# use this as the default docker-compose yaml definition
COMPOSE_FILE_BASE=compose/compose-org2.yaml
COMPOSE_FILE_ORG2=compose/${CONTAINER_CLI}/docker-compose-org2.yaml
# certificate authorities compose file
COMPOSE_FILE_CA_BASE=compose/compose-ca-org2.yaml
COMPOSE_FILE_CA_ORG2=compose/${CONTAINER_CLI}/docker-compose-ca-org2.yaml
# database
DATABASE="couchdb"
