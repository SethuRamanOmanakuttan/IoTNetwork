...
function networkDown() {
  ...
  COMPOSE_ORG2_BASE_FILES="-f addOrg/compose/${COMPOSE_FILE_ORG2_BASE} -f addOrg/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG2_BASE}"
  COMPOSE_ORG2_COUCH_FILES="-f addOrg/compose/${COMPOSE_FILE_ORG2_COUCH} -f addOrg/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG2_COUCH}"
  COMPOSE_ORG2_CA_FILES="-f addOrg/compose/${COMPOSE_FILE_ORG2_CA} -f addOrg/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG2_CA}"
  COMPOSE_ORG2_FILES="${COMPOSE_ORG2_BASE_FILES} ${COMPOSE_ORG2_COUCH_FILES} ${COMPOSE_ORG2_CA_FILES}"

  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG2_FILES} down --volumes --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG2_FILES} down --volumes
  else
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addOrg/fabric-ca/org2/msp addOrg/fabric-ca/org2/tls-cert.pem addOrg/fabric-ca/org2/ca-cert.pem addOrg/fabric-ca/org2/IssuerPublicKey addOrg/fabric-ca/org2/IssuerRevocationPublicKey addOrg/fabric-ca/org2/fabric-ca-server.db'
...
CC_NAME="IoTDataContract"
# chaincode path defaults to "NA"
CC_SRC_PATH="iot-chaincode"
...
# use this as the default docker-compose yaml definition for org2
COMPOSE_FILE_ORG2_BASE=compose-org2.yaml
# use this as the docker compose couch file for org2
COMPOSE_FILE_ORG2_COUCH=compose-couch-org2.yaml
# certificate authorities compose file
COMPOSE_FILE_ORG2_CA=compose-ca-org2.yaml
#
